...
const ChatHistory = () => {
      ...
      const handleHistory = async () => {
        try {
          const response = await axios.get(
            `http://localhost:5000/history?email=${session.data.user.email}`
          );
          console.log("Message sent successfully:", response.data.history);
  ...
  }, [session.status, session.data?.user?.email, history]);
  // console.log(session);
  const handleNewChat = () => {
    setChatlog([]);
  };
  const handleHistory = (item) => {
    const userMessage={user: "me", message:item.query}
    const gptMessage = { user: "gpt", message: item.response };
    setChatlog([userMessage,gptMessage]);
  };
              ...
              <div className="dark:hover:bg-[#212121] hover:bg-[#d2cfcf] rounded-lg text-black dark:text-white flex flex-row justify-center items-center m-[1rem]">
                <div className="mb-[0.5rem] mt-[0.5rem] w-[2.5rem] h-[2.5rem] flex justify-center rounded-full dark:bg-white bg-black dark:text-gray-950 text-white">
                  <svg
                    ...
                    className="h-3/4 w-3/4 mt-[0.3rem]"
                    role="img"
                  >
                    <path
                ...
                <div className="mb-[0.5rem] ml-[1rem] mt-[0.5rem]">
                  New Chat
                </div>
              ...
              {history.length > 0 &&
                history.map((item, index) => (
                  <Button
                    ...
                    className="w-[13rem] mr-[1rem] mt-[0.5rem] border border-gray-700"
                    key={index}
                    title={item.query} // Add title attribute to show full text on hover
                  >
